...
local conf, pconf
XPerl_RequestConfig(function(new) conf = new pconf = conf.player if (XPerl_Player) then XPerl_Player.conf = conf.player end end, "$Revision: 412 $")
...
local function XPerl_Player_DruidBarUpdate(self)
	...
	if select(4, _G.GetBuildInfo()) >= 40000 then
		if select(5, GetTalentTabInfo(1)) >= 31 then
			self.eclipse:Show()
		else
			self.eclipse:Hide()
		end
